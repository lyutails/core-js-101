...
function getFizzBuzz(num) {
  if (num % 3 === 0 && num % 15 !== 0) return 'Fizz';
  if (num % 5 === 0 && num % 15 !== 0) return 'Buzz';
...
function doRectanglesOverlap(rect1, rect2) {
  // eslint-disable-next-line no-mixed-operators
  if (rect1.top > rect2.width || rect2.top > rect1.width) return false;
  if (rect1.left > rect2.height || rect2.left > rect1.height) return false;
  return true;
}
...
function isInsideCircle(circle, point) {
  const difX = Math.abs(circle.center.x - point.x);
  const difY = Math.abs(circle.center.y - point.y);
  const { radius } = circle;
  if (Math.hypot(difX, difY) < radius) return true;
  return false;
}
...
function findFirstSingleChar(str) {
  ...
  for (let i = 0; i < str.length; i++) {
    if (characters.filter((j) => j === str.charAt(i)).length === 1) return str.charAt(i);
  }
  return null;
}
...
function isCreditCardNumber(ccn) {
  let sum = 0;
  let ccnDigits = ccn.toString().split('');
  ccnDigits = ccnDigits.map(Number);
  // eslint-disable-next-line no-plusplus
  for (let i = 0; i < ccn.length; i++) {
    // eslint-disable-next-line radix
    let digit = ccnDigits[i];
    if (i % 2 === 0) {
      digit *= 2;
      if (digit > 9) {
        digit -= 9;
...
function getCommonDirectoryPath(pathes) {
  const array = pathes.concat().sort();
  // eslint-disable-next-line prefer-destructuring
  const a1 = array[0];
  const a2 = array[array.length - 1];
  const arrayLength = a1.length;
  let i = 0;
  while (i < arrayLength && a1.charAt(i) === a2.charAt(i))
  // eslint-disable-next-line no-const-assign, no-plusplus, brace-style
  { i++; }
  return a1.substring(0, i).slice(0, a1.indexOf('/'));
}
